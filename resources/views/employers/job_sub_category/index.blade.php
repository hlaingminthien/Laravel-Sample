        ...
        <table class="table table-bordered cmp_table" id="dtBasicExample">
            ...
            @foreach($job_sub_categories as $job_sub_category)
              <tr>
                ...
                <td>{{$job_sub_category->categoryName}}</td>
                <td>
                  <a href="{{ route('employeers.job_sub_categories.edit',['id' => $job_sub_category->id])}}" class="btn-added">
                    <i class="lni-pencil"></i>Edit</a>
                    
                </td>
                <td>
                  <a href="#" data-toggle="modal" data-target  ="#delete_{{$job_sub_category->id}}" class="btn-added">
                   <i class="lni-trash"></i>Delete</a>
                </td>
              </tr>
            @include('employers.partials.delete',['id'=>$job_sub_category->id])
            @endforeach
            </tbody>
          </table>

          <form action="{{route('employeers.job_sub_categories.destory')}}" method="post" id="my_delete_hidden_form">
          @csrf
